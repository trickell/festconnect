...
<div class="relative min-h-screen py-24 bg-black text-white overflow-hidden" x-data="{ tab: 'info' }">
    ...
    <div class="relative z-10 container mx-auto px-4 max-w-6xl">
        <!-- Penalty Alerts (Visible only to OWNER or ADMIN) -->
        @if(count($penalties) > 0 || ($user->penalty_marks > 0))
            <div class="mb-12 space-y-4">
                <h1
                    class="text-2xl md:text-4xl font-black italic uppercase tracking-tighter mb-2 bg-clip-text text-transparent bg-gradient-to-r from-red-500 via-blue-500">
                    Penalties & Standing
                </h1>

                <!-- Penalty Marks Summary -->
                ...
                @foreach($penalties as $penalty)
                    <div
                        class="bg-red-500/10 border border-red-500/20 p-6 rounded-3xl flex items-start gap-4 animate-pulse-subtle shadow-lg shadow-red-500/5">
                        ...
                        <div class="flex-grow">
                            ...
                            <p class="text-gray-300 text-sm italic mt-1 font-medium italic">
        ...
        @if($isOwner)
            <div class="flex gap-4 mb-12 border-b border-white/10 pb-6 relative z-10">
                ...
                <button @click="tab = 'posts'"
                    ...
                    class="px-8 py-3 rounded-2xl font-black uppercase tracking-widest transition text-xs">Manage My
                    Posts</button>
            </div>
        @endif
        ...
        <div x-show="tab === 'info'" x-cloak class="animate-fade-in-up">
            <div id="profile_display_section" class="space-y-12">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach($user->genres as $genre)
                                        <span
                                            class="px-4 py-2 bg-gradient-to-tr from-pink-500/20 to-purple-500/20 border border-white/10 rounded-full text-xs font-bold text-white uppercase tracking-tighter">{{ $genre }}</span>
                                    @endforeach
                                </div>
                            </div>
                        @endif
                                ...
                                <div
                                    ...
                                    <div>
                                        ...
                                        <textarea name="about_me" rows="4"
                                            class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-purple-500 transition resize-none">{{ $user->about_me }}</textarea>
                                    </div>
                                    <div>
                                        <label
                                        ...
                                        <textarea name="festivals" rows="3"
                                            class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-purple-500 transition resize-none">{{ $user->festivals }}</textarea>
                                    </div>
                                    <div>
                                        <label
                                        ...
                                        <div class="space-y-6">
                                            <div>
                                                ...
                                                <div class="flex flex-wrap gap-2" id="edm_genres_wrap">
                                                    @foreach(['Techno', 'House', 'Dubstep', 'Trance', 'Drum & Bass', 'Hardstyle', 'Melodic Dubstep', 'Psytrance', 'Future Bass', 'Trap'] as $g)
                                                        <button type="button" data-genre="{{ $g }}"
                                                            class="genre-chip px-3 py-1.5 border border-white/10 rounded-lg text-[10px] font-bold uppercase transition {{ in_array($g, (array) $user->genres) ? 'bg-purple-600 border-purple-500 text-white' : 'bg-white/5 text-gray-500 hover:border-white/30' }}">{{ $g }}</button>
                                                    @endforeach
                                                </div>
                                            </div>
                                            <div>
                                                ...
                                                <div class="flex flex-wrap gap-2" id="rock_genres_wrap">
                                                    @foreach(['Alternative', 'Metal', 'Punk', 'Indie', 'Classic Rock', 'Hard Rock', 'Grunge', 'Progressive Rock', 'Psych Rock', 'Shoegaze'] as $g)
                                                        <button type="button" data-genre="{{ $g }}"
                                                            class="genre-chip px-3 py-1.5 border border-white/10 rounded-lg text-[10px] font-bold uppercase transition {{ in_array($g, (array) $user->genres) ? 'bg-pink-600 border-pink-500 text-white' : 'bg-white/5 text-gray-500 hover:border-white/30' }}">{{ $g }}</button>
                                                    @endforeach
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="flex gap-4 pt-4">
                                        <button type="submit"
                                            class="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 py-4 rounded-2xl font-black uppercase tracking-widest transition shadow-lg active:scale-95">Save
                                            Profile</button>
                    ...
                    <div class="space-y-8">
                        <div
                            class="bg-gradient-to-br from-indigo-600/20 to-purple-600/20 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            ...
            <div x-show="tab === 'posts'" x-cloak class="animate-fade-in-up space-y-6" x-data="{ 
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                    ...
                    <div class="flex items-center gap-3 bg-white/5 p-2 rounded-2xl border border-white/10">
                        <span class="text-[10px] font-black uppercase tracking-widest text-gray-500 ml-2">Sort By:</span>
                ...
                <div class="grid grid-cols-1 gap-4">
                    <template x-for="post in sortedPosts" :key="post.id">
                        <div x-data="{ expanded: false }"
                            ...
                            <!-- Expanded Content -->
                            <div x-show="expanded" x-collapse x-cloak class="p-6 pt-0 border-t border-white/5 bg-black/20">
                                <!-- Images -->
