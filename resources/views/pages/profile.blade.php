...
@section('content')
<div class="relative min-h-screen py-24 bg-black text-white overflow-hidden">
    <!-- Video Background -->
            ...
            <div class="mb-12 space-y-4">
                @foreach($penalties as $penalty)
                    <div
                        ...
                        <div class="flex-grow">
                            <h4 class="font-black uppercase tracking-widest text-red-500 text-sm">Account Warning
                                @if($penalty->status === 'pending') (Pending Review) @endif
                            </h4>
                            <p class="text-gray-300 text-sm italic mt-1 font-medium">
                                "{!! nl2br(e($penalty->admin_comment ?: $penalty->reason)) !!}"</p>
                        </div>
                    </div>
                @endforeach
            </div>
                                    ...
                                    <div
                                        ...
                                        <div>
                                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Flagged
                                                {{ $flag->target_type }} #{{ $flag->target_id }}</p>
                                            <p class="text-sm text-gray-200 mt-1 italic">"{!! Str::limit($flag->reason, 80) !!}"
                                            </p>
                                        </div>
                                        <span
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <div class="space-y-6">
                                        ...
                                        <div>
                                            <h4 class="text-[10px] font-bold text-pink-400 uppercase tracking-[0.2em] mb-3">
                                                Rock
                                                Genres</h4>
    ...
    <div id="mod_flag_modal" class="fixed inset-0 z-[100] hidden flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
        <div class="bg-gray-900 border border-white/10 rounded-3xl w-full max-w-lg p-8 shadow-2xl animate-fade-in-up">
            ...
            <form id="mod_flag_form" class="space-y-6">
                ...
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Flag Type</label>
                    <select name="type" class="w-full bg-black/40 border border-white/10 rounded-xl p-4 text-white focus:border-pink-500 outline-none">
                        <option value="warning">Account Warning / Bad Conduct</option>
                        <option value="good">Remarkable User / Good Behavior</option>
                    </select>
                </div>

                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2">Reason (Visible to Admin)</label>
                    <textarea name="reason" rows="4" required class="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-gray-200 focus:outline-none focus:border-pink-500 transition resize-none" placeholder="Reason for this flag..."></textarea>
                </div>

                <div class="flex gap-4">
                    <button type="submit" class="flex-1 bg-pink-600 hover:bg-pink-500 py-4 rounded-2xl font-black uppercase tracking-widest transition text-sm shadow-xl">Submit Flag</button>
                    <button type="button" onclick="closeModFlagModal()" class="px-8 bg-white/5 hover:bg-white/10 py-4 rounded-2xl font-bold uppercase tracking-widest transition text-gray-400 text-sm">Cancel</button>
                </div>
            </form>
        </div>
    </div>
